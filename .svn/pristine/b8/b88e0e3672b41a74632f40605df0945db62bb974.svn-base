...
bool loadPCD(char *name, pcType cloud_ptr)
{
  ...
  {
    std::cout << "Filename " << name << " is not a valid file." << std::endl;
    return false;
  }
  bool result = (pcl::io::loadPCDFile(name,*cloud_ptr) == 0);
  if(result)
    std::cout << "Loaded file " << name << " with " << cloud_ptr->size() << " points."; cout << std::endl;
  return result;
}

void fillInIndices(std::vector<int> &indices, int start = 0, int end = -1, bool push_back = true) //non-inclusive end!!
{
  if(end<0)
  {
    push_back = false;
...
int main(int argc, char *argv[])
{
  ...
  char *skill_names[] = {"close the box","pour"};//{"test_close the box","test_pour"};//{"close the box", "insert", "place"};
  char demo_base[] = "demo";
  char kf_base[] = "kf";
  char separator[] = "_";
  char ext[] = "pcd";
  ...
  bool display_contour = false;
  bool display_clusters = false;
  bool filter_pc_for_view = false;
  ...
  while (!viewer->wasStopped ())
  {
    ...
    switch (keyPress)
    {
      ...
      case '2':
        ...
        break;
      case '3':
        filter_pc_for_view = !filter_pc_for_view;
        break;
      default:
      break;
    }
    ...
    if(give_info)
    {
      ...
      if(viz_ind_clusters)
      {
          ...
          std::cout << std::endl;
          std::cout << feats[selectedCluster].color[0] << " " << feats[selectedCluster].color[1] << " " << feats[selectedCluster].color[2];
          std::cout << std::endl;
      }
    ...
    if(process_pc)
    {
      ...
      if(!isCls)
      {
        ...
        float threshs[]  = {-0.4,0.4,-0.2, 0.2, 0.9, 1.4};
        if(is_set_z_thresh)
        {
            cin >> threshs[4];
            cin >> threshs[5];
      ...
      else
      {
        ...
        if(!isCls)
          segmenter.segmentPointCloud(filtered_cloud, clusters, cluster_normals, contour);
        else
    ...
    if(viz_proc_result)
    {
      ...
      if(display_contour)
        segmenter.displayPlane(contour);
      if(display_clusters)
      {
